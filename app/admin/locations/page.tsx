  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-5">
        <h2 className="mb-3 text-base font-semibold text-gray-900">{st(lang, "adm_loc_add")}</h2>
        <form action={createLocation} className="grid gap-3 sm:grid-cols-2">
          <label className="text-sm sm:col-span-2">
            <span className="mb-1 block text-gray-600">{st(lang, "adm_loc_name")}</span>
            <input name="name" required className={inputCls} />
          </label>
          <label className="text-sm">
            <span className="mb-1 block text-gray-600">{st(lang, "adm_loc_lat")}</span>
            <input name="lat" type="number" step="any" required className={inputCls} />
          </label>
          <label className="text-sm">
            <span className="mb-1 block text-gray-600">{st(lang, "adm_loc_lng")}</span>
            <input name="lng" type="number" step="any" required className={inputCls} />
          </label>
          <label className="text-sm">
            <span className="mb-1 block text-gray-600">{st(lang, "adm_loc_radius")}</span>
      ...
      {locations.length === 0 ? (
        <p className="text-sm text-gray-500">{st(lang, "adm_loc_empty")}</p>
      ) : (
            ...
            <div key={l.id} className="rounded-lg border border-gray-200 bg-white p-4">
              <form action={updateLocation} className="grid items-end gap-3 sm:grid-cols-12">
                ...
                <label className="text-sm sm:col-span-4">
                  <span className="mb-1 block text-xs text-gray-500">{st(lang, "adm_loc_name")}</span>
                  <input name="name" defaultValue={l.name} required className={inputCls} />
                </label>
                <label className="text-sm sm:col-span-3">
                  <span className="mb-1 block text-xs text-gray-500">{st(lang, "adm_loc_lat")}</span>
                  <input name="lat" type="number" step="any" defaultValue={l.lat} required className={inputCls} />
                </label>
                <label className="text-sm sm:col-span-3">
                  <span className="mb-1 block text-xs text-gray-500">{st(lang, "adm_loc_lng")}</span>
                  <input name="lng" type="number" step="any" defaultValue={l.lng} required className={inputCls} />
                </label>
                <label className="text-sm sm:col-span-2">
                  <span className="mb-1 block text-xs text-gray-500">{st(lang, "adm_loc_radius")}</span>
                ...
                <div className="flex items-center gap-2 sm:col-span-12">
                  <button
                    type="submit"
                    className="rounded-md border border-gray-300 px-3 py-1.5 text-sm hover:border-brand hover:text-brand"
                  >
              ...
              <form action={deleteLocation} className="mt-2 border-t border-gray-100 pt-2">
                ...
                <button
                  ...
                  className="rounded-md border border-gray-300 px-3 py-1 text-xs text-gray-600 transition hover:border-red-400 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-40"
                >
